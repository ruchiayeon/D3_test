...
function TeamChart() {
  ...
  const [checkednodes, setCheckedNodes] = React.useState<Map<string, IChecked>>(
    new Map()
  );
  const [nodeHeight, setNodeHeight] = React.useState<number>(0);
  const [nodewidth, setNodeWidth] = React.useState<number>(0);
  const getStore = useSelector((state: { groups: ChartProps }) => state.groups);
        ...
        (d) =>
          `${
            d.isChildOpen || !d.children
              ? `M${d.depth * getStore.nodeSize.height - 27} 8.5 H${
                  d.depth * getStore.nodeSize.height - 27
                } ${d.depth * getStore.nodeSize.height - 18}`
              : `M${d.depth * getStore.nodeSize.height - 22.5} 4 L${
  ...
  function TreeAcodianTranslate() {
    ...
    setNodeHeight(
      (nodes.filter((n) => n.isOpen).length + 2) * getStore.nodeSize.height
    );
    setNodeWidth(
      200 +
    ...
    nodes
      ...
      .map((v) =>
        v.eachBefore((cv) => {
          ...
          d3.select(document.getElementById(`${cv.data.groupCode} groupItem`))
            .transition()
          ...
          d3.select(document.getElementById(`${cv.data.groupCode} groupText`))
            .transition()
            .duration(getStore.duration)
            .style("opacity", 1)
            .style(
